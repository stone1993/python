...
__author__ = 'xl'
"""
__date__ = TIME： 2019/01/07 2:59 PM
describe: 本节内容 来自 pep 380
...
try:
    ...
    while 1:
        try:
            _s = yield _y
        except GeneratorExit as _e:     #捕获到close 产生的
            try:
                _m = _i.close           #获得close 方法,如果只是迭代器,不支持close 方法 触发 AttributeError 执行空操作
            except AttributeError:
                pass
            else:
                _m()
            raise _e                    #将异常 逐级上报
        except BaseException as _e:     # BaseException 获取所有的异常
            _x = sys.exc_info()
            try:
                _m = _i.throw           # 如果只是迭代器,不支持throw 方法 触发 AttributeError 直接将异常上报
            except AttributeError:
                raise _e
            else:
                try:                    # 如果是生成器 具有 throw 方法,调用 该方法 ，调用过程中可能产生 StopIteration 异常
                    _y = _m(*_x)
                except StopIteration as _e:
                    _r = _e.value
                    break
            ...
            try:
                if _s is None:  #如果 yield from 获得的参数为  None 时 ，调用 next 方法
                    _y = next(_i)
                else:           #否则 调用 send 方法
                    _y = _i.send(_s)
            except StopIteration as _e:
